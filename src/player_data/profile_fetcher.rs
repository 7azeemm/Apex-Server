...
pub async fn get_player_profile(username: &str, player_uuid: &str, profile_name: Option<String>) -> Result<PlayerProfile, Box<dyn Error>> {
    ...
    for profile in profiles {
        ...
        if members.contains_key(&undashed_uuid) {
            ...
            if is_selected { selected_profile = Some(cute_name.to_owned()) }
...
fn scan_storage(data: &Value) -> Storage {
    ...
    if let Some(backpacks) = &inventory.get("backpack_contents").and_then(|v| v.as_object()) {
    ...
    if let Some(sacks_counts) = inventory.get("sacks_counts").and_then(|v| v.as_object()) {
...
fn scan_setups(storage: &Storage) -> HashMap<SetupType, PlayerSetup> {
    ...
    for setup_type in setups_list {
        ...
        let armor_ids: HashSet<String> = armor.iter().filter_map(|item| item.as_ref()).map(|(item_id, _)| item_id.clone()).collect();
