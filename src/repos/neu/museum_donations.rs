...
pub async fn load_museum_donations() {
    let start_time = Instant::now();

    match load_file("constants/museum.json").await {
        Ok(Value::Object(items)) => {
            let required_keys = ["weapons", "armor", "rarities", "children", "itemToXp", "sets_to_items", "set_exceptions"];

            for key in required_keys {
                if !items.contains_key(key) {
                    println!("[NEU-Repo] Museum data is missing from repo!");
                    return;
                }
            }
            ...
            *set_exceptions_lock = set_exceptions.iter().filter_map(|(k, v)| v.as_str().map(|s| (s.to_string(), k.clone()))).collect();
            ...
            println!("[NEU-Repo] Loaded museum donations in {:.2?}", start_time.elapsed());
        }
        Err(err) => println!("[NEU-Repo] Error occurred while loading museum donations: {:?}", err),
        _ => println!("[NEU-Repo] Error occurred while loading museum donations: Invalid JSON format")
    }
}
