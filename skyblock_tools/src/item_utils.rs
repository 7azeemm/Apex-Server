...
pub fn is_pet(item_nbt: &ItemNbt) -> bool {
    if let Some(Value::String(id)) = item_nbt.get_extra_map().and_then(|m| m.get("id")) {
        return id == "PET";
    }
...
pub fn get_item_id(item_nbt: &ItemNbt) -> Option<String> {
    ...
    match id {
        ...
        _ => Some(match STARRED_ITEMS_INGREDIENT.contains_key(id) {
...
pub fn decode_item(encoded_str: &str) -> Result<ItemNbt, Box<dyn Error>> {
    ...
    let i0 = match root {
        Value::Compound(mut root_map) => match root_map.remove("i") {
...
pub fn decode_items(
    encoded: &str,
    keep_empty_slots: bool,
) -> Result<Vec<Option<ItemNbt>>, Box<dyn Error>> {
