...
const FASTER_ENDPOINT: &str = "https://playerdb.co/api/player/minecraft";
...
pub async fn get_player_uuid(player_name: &str) -> Option<String> {
    ...
    let url = format!("{FASTER_ENDPOINT}/{player_name}");
    let id = match send_http_request(&url).await.map(|s| s.get_str("data/player/id").map(|s| s.to_owned())) {
        Ok(Some(id)) => Some(id),
        _ => match send_http_request(&format!("{NAME_TO_UUID_URL}/{player_name}")).await {
            Ok(json) => json.get_str("id").map(|s| s.to_owned()),
            Err(error) => {
...
pub async fn get_player_username(player_uuid: &str) -> Option<String> {
    ...
    let url = format!("{FASTER_ENDPOINT}/{player_uuid}");
    let username = match send_http_request(&url).await.map(|v| v.get_str("data/player/username").map(|s| s.to_owned())) {
        Ok(Some(username)) => Some(username),
        _ => match send_http_request(&format!("{UUID_TO_NAME_URL}/{player_uuid}")).await {
            Ok(json) => json.get_str("name").map(|s| s.to_owned()),
            Err(error) => {
