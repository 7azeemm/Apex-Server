    ...
    if let Some(dungeons) = profile_data.get("dungeons") {
        if let Some(dungeon_types) = dungeons.get("dungeon_types") {
            if let Some(catacombs) = dungeon_types.get("catacombs") {
                ...
                let master_floors = dungeon_types.get("master_catacombs")
                    .and_then(|mc| Some(extract_floor_completions(mc.get_object("milestone_completions"))));
...
pub async fn get_events_info(pdr: &mut PlayerDataResponse) {
    ...
    if let Some(election) = mayor_info.election() {
        ...
        let total_votes: u64 = election.iter()
            .filter_map(|(_, votes)| *votes)
            .sum();
...
fn get_level_and_progress(xp_table: &[u64], xp: u64, default: u64, max: u64, cap: Option<u64>) -> (u64, String) {
    ...
    if level < max && level < cap.unwrap_or(u64::MAX) {
        ...
        let next_lvl_xp = xp_table.get(level as usize + 1).cloned().unwrap_or(curr_lvl_xp);
