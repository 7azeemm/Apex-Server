...
impl ModifierHandler for PetModifier {
    async fn calculate_value(&self, _: &str, modifier: &Value, item_value: &mut ItemValue) {
        let Value::String(pet_info) = modifier else { return };
        ...
        if let Some(skin) = pet_data.get("skin") {
            if let Some(skin_name) = skin.as_str() {
                ...
                item_value.add_modifier(&*id, Modifier::new_one(price, ModifierInfo::new("Pet Skin", get_readable_name(&skin_name))));
            }
        }

        if let Some(held_item) = pet_data.get("heldItem") {
            if let Some(held_item_name) = held_item.as_str() {
                let price = get_shared_lowest_bin(&held_item_name).await;
                item_value.add_modifier(&*held_item_name, Modifier::new_one(price, ModifierInfo::new("Held Item", get_readable_name(held_item_name))));
            }
        }
    }
}