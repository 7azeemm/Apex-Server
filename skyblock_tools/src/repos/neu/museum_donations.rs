...
pub async fn load_museum_donations() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    for set_id in armor {
        ...
        let item_ids = sets_to_items.get_array(set_id).map(|a| {
            a.iter().filter_map(|i| i.as_str().map(String::from)).collect::<Vec<String>>()
        });
    ...
    *set_exceptions_lock = set_exceptions
        .iter()
        .filter_map(|(k, v)| v.as_str().map(|s| (s.to_string(), k.clone())))
        .collect();
