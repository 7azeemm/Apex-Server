...
pub async fn get_accessory_upgrades(pdr: &PlayerDataResponse, sb: &mut StringBuilder, page: u64, soulbound: bool) {
...

    {
        ...
        for (id, rarity) in all_accessories.iter() {
            ...
            let Some(mut upgrade_line) = get_talisman_upgrades(id).await else {
    ...
    let player_has_hat = player_talismans
        .iter()
        .any(|(id, _)| id.starts_with("PARTY_HAT_") || id.starts_with("BALLOON_HAT_"));
    let mut kept_one_hat = false;
    let mut to_remove = Vec::new();
    let mut to_add = Vec::new();

    for talisman in talismans.keys() {
        let piggy_banks = ["PIGGY_BANK", "CRACKED_PIGGY_BANK", "BROKEN_PIGGY_BANK"];
        if piggy_banks.contains(&talisman.as_str()) {
            to_remove.push(talisman.to_owned());
            if !piggy_banks.iter().any(|piggy| player_talismans.contains_key(*piggy)) {
    ...
    for (id, rarities) in SPECIAL_TALISMANS {
        let Some(talisman) = player_talismans.get(*id) else {
            talismans.remove(*id);
        ...
        let rarity_index = rarities
            .iter()
            .position(|r| r == &talisman.rarity)
            .unwrap_or_default();
    ...
    valuable_talismans.sort_by(|a, b| match (a.coins_per_mp, b.coins_per_mp) {
    ...
    let total_pages = list.len().div_ceil(ITEMS_PER_PAGE);
    ...
    for (i, talisman) in list.iter().skip(start).take(ITEMS_PER_PAGE).enumerate() {
        ...
        match soulbound {
            ...
            false => {
                ...
                if let Some(downgrade) = &talisman.downgrade && let Some(price) = &downgrade.price {
                    let name = get_pretty_name(&downgrade.id);
                    let rarity = get_pretty_name(&downgrade.rarity);
                    sb.push(format!("   Upgrades From: {} ({}, {} coins)", name, rarity, format_number(*price)));
                    sb.push(format!("   Net Cost: {} coins", format_number(price.saturating_sub(*price))));
                }
            }
        }
    }
...
fn get_magical_power(talisman: &Talisman, with_recomb: bool) -> u64 {
    ...
    let mp = MAGICAL_POWER.get(&rarity).copied().unwrap_or(1);
