...
async fn update_mayors_info() -> Result<(), Box<dyn Error>> {
    ...
    // Parse minister
    let minister = mayor_data.get_object("minister")
        .map(|minister_data| {
            ...
            if let Some(perk) = minister_data.get("perk").and_then(|v| v.as_object()) {
                if let (Some(name), Some(desc)) = (perk.get("name"), perk.get("description")) {
                    perks.insert(name.as_str().unwrap_or("Unknown").to_owned(), desc.as_str().unwrap_or("Unknown").to_owned());
                }
            }

            Mayor::new(
                minister_data.get("name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_owned(),
                perks,
            )
        });

    // Parse election candidates
    let election = json.get_object("current")
        .and_then(|election_data| election_data.get("candidates").and_then(|v| v.as_array()))
        .map(|candidates| {
            let mut candidates_vec: Vec<(Mayor, Option<u64>)> = candidates.iter()
...
pub fn get_skyblock_date() -> String {
    ...
    format!("Year {} Month {} Day {} {:02}:{:02}:{:02}",
            ...
            sb_date.get_hour(), sb_date.get_min(), sb_date.get_sec())
}
...
pub fn get_special_mayors_info() -> String {
    ...
    infos.iter().map(|(_, s)| s.clone()).collect::<Vec<String>>().join("\n")
}
